...
const AmazonToolkitStore = () => {
  ...
  const deployFromGithub = async () => {
    ...
    try {
      setGithubApp(prev => ({ ...prev, deploying: true }));

      const response = await fetch('/api/deploy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          repoUrl: githubApp.repoUrl,
          appName: githubApp.name
        })
      });
      ...
      setGithubApp(prev => ({ ...prev, deploying: false }));
    }
  };
                ...
                {addMode === 'github' && (
                  <div className="space-y-4">
                    <div className="p-4 rounded-lg" style={{ background: 'rgba(251, 191, 36, 0.1)', border: '2px solid rgba(251, 191, 36, 0.3)' }}>
                      <p className="text-sm text-yellow-800">⚠️ Token Vercel requis dans les variables d’environnement</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block font-semibold mb-2">Nom *</label>
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-2">
                      <a
                        href={app.vercel_url || (SUPABASE_URL ? `${SUPABASE_URL}/storage/v1/object/public/${app.storage_path}` : '#')}
                        target="_blank"
